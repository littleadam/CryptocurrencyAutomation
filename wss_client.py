...
def onClose(ws, error):
    print("Closing:")
    print(error)

def onMessage(ws, error):
    print("Message")
    print(error)
...
class DeltaRestClient:
  ...
  def wss_app(self, method, path, payload=None, query=None, auth=False, base_url=None, headers={}):
    if base_url == None:
      base_url = self.base_url
    url = '%s%s:2096' % (base_url, path)
    ...
    if auth:
      ...
      ws = websocket.WebSocketApp('wss://api.delta.exhange:2096',on_message=onMessage,on_error=onError,on_close=onClose,on_open=onOpen)
      print(ws)
  ...
  def auth_connection(self,auth=True):
    response = self.wss_app('GET', '/live', auth=auth)
  ...
  def get_transactions(self, asset_id):
    response = self.request("GET", "/v2/wallet/transactions", auth=True) #query={'asset_id': asset_id}, auth=True)
    wallets = parseResponse(response)
    return(wallets)
  ...
  def place_order(self, product_id, size, side, limit_price=None, time_in_force=None, order_type="market_order", post_only='false', client_order_id = None):
    order = {
      'product_id': product_id,
      'size': int(size),
      'side': side,
